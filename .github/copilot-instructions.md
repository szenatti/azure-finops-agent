...
- A process-wide `SemaphoreSlim` additionally caps concurrent chat turns (`Chat:MaxConcurrentTurns`, default 6) with a 20-second admission queue; rejected callers get the `busy` SSE event. Like the turn gate and the cost-query coordinator, it is per-process, so multi-instance hosting would not share it.
...
- It queries with `granularity: None` and no dataset filter, so by default it returns one undifferentiated total per subscription. Set `groupBy` to a supported Cost Management dimension (`ServiceName`, `ResourceGroupName`, `MeterCategory`, `MeterSubCategory`, `ResourceLocation`, `ResourceType`, `ChargeType`, `PublisherType`, `SubscriptionName`) for a cross-subscription breakdown in `byGroup`, keyed by group and currency; unknown dimensions are rejected before any call and `groupBy` disables the management-group aggregate. It still cannot express a daily series or a spike/trend, so route those to a single scoped `QueryAzure` Cost Management query with `granularity="Daily"`; do not add granularity to the estate-total tool, whose parsers sum every returned row.
- The tool needs no billing-account access and is always available. An empty `Microsoft.Billing/billingAccounts` response is never a reason to refuse an estate-wide question.
...
- It tries one supplied, verified containing management-group query, then queries subscriptions sequentially until `AzureQueryTools.InteractiveCostScopeBudget` (240s) elapses. Each Cost Management query takes roughly 3-4 seconds, so one call covers about 60 scopes. Successful per-scope responses are cached for 30 minutes (`HttpHelper.CostResponseCacheTtl`) and replay without touching the tenant gate, so repeated calls accumulate coverage instead of restarting; the cache must outlive several budgets or resumed calls thrash. Unattempted scopes say so. Estates larger than a few budgets need a billing-account scope or Cost Management exports. Paginated cost responses cannot be accepted as complete totals.
- Partial coverage is not a proportional sample of the estate. Never scale a partial total up, and never present it as representative — the covered scopes are whichever discovery returned first.
