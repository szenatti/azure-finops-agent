...
Security checks cover the restricted runtime/host-tool permission policy, ownerless and expired artifacts, public HTTPS destinations/redirects/cancellation, legacy and revoked browser tickets, absolute expiry and account restoration, shared workload admission and retained upload reservations. A synthetic text upload runs the real embedded inspector, so Python 3 is required. Targeted flags are `--runtime-policy-only`, `--artifact-policy-only`, `--web-policy-only`, `--auth-policy-only` and `--quota-policy-only`; each runs the security checks through that section. The default command continues through every existing regression. Deployment defaults and remaining live-runtime checks are documented in [Security Hardening](../../docs/security-hardening.md).
